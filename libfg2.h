...
#define FG_MAJOR_VERSION 0
#define FG_MINOR_VERSION 3
#define FG_MICRO_VERSION 0

#define FG_CHECK_VERSION(Maj, Min, Mac) ( \
...
extern const int fg_major_version;
extern const int fg_minor_version;
extern const int fg_micro_version;

int fg_check_version(int maj, int min, int mic);

#define FG_JPEG_SUPPORT 0
...
#define FG_FORMAT_YUYV          V4L2_PIX_FMT_YUYV

#define FG_FORMAT_DEFAULT       FG_FORMAT_YUYV
...
//self packaged camera capbilitys
typedef struct 
{
...
#define MAX_NUM_MMP_BUFS (5)
///
/// This stores information associated with a frame grabber device.
///
typedef struct
{
    fg_device_info          curr_device;        ///< current Device struct, eg. "/dev/video0"
    int                     fd;             ///< File handle for open device
    GList                  *camera_caps_list;///< List pointed to fg_camera_cap, that indicate all caps of current device
    fg_caps                 caps;           ///< Device capabilities
    fg_input                *inputs;        ///< Input sources (eg. TV, SVideo), only support 1 now
    int                     num_inputs;     ///< Number of input sources , only support 1 now
    ...
    fg_format               format;         ///< Current Video format (size, etc)
    fg_control              *controls;      ///< Video controls (hue, etc)
    int                     num_controls;   ///< Number of controls
    GList                   *devices_list;  ///< fg_device_info list , list all camera devices
...
//------------------------------------------------------------------------------
///
/// \brief  init a new frame grabber.
///
/// This function init a frame grabber before open the device specified
///
///
/// \return a new frame grabber if success , else return null
fg_grabber *fg_init();
...
///                 only support path now , will support friendly name later
///
...
/// \param  fr  Frame grabber device to store capture image.

///
/// \return If a new frame could be successfully allocated, initialized and
///         read from the frame grabber, 0 returned.
...
//------------------------------------------------------------------------------
///
/// \brief  Set the selected video fg_format.
///
...
//------------------------------------------------------------------------------
///
/// \brief  Set the selected video format, only format , with and height will kee same as original.
///
/// This function sets the video format of the currently active input.  The
/// formats are of a select few; FG_FORMAT_RGB24, FG_FORMAT_BGR24,
/// FG_FORMAT_YUV420, or FG_FORMAT_YVU240.  The libv4l library is used to
...
//------------------------------------------------------------------------------
        ...
        int fg_frame_save( fg_frame* fr, FILE *outfile);
		...
		///
		int fg_frame_save_Jpeg(fg_frame* fr, const char* filename);
#endif
