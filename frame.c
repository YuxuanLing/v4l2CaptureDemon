...
int fg_frame_save( fg_frame* fr, FILE *outfile)
{
    ...
    if ( outfile == NULL )
    {
        fg_debug_error("fg_frame_save(): output file handle is null");
        return -1;
    }
    ...
    {
        fg_debug("fg_frame_save(): saving format is RGB24");
        fwrite( fr->data,  1, fr->length, outfile);                   
    }else if(fr->format == FG_FORMAT_YUV420) {
        fg_debug("fg_frame_save(): saving format is YUV420");
        fwrite( fr->data,  1, fr->length, outfile);                       
    }else if(fr->format == FG_FORMAT_YUYV) {
        fg_debug("fg_frame_save(): saving format is YUYV");
        fwrite( fr->data,  1, fr->length, outfile);                       
    }
